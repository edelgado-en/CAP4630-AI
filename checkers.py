...
class Checker(TwoPlayerGame):
    ...
    def make_move(self, pos):
        ...
        ------
        """
        # create these variables to improve readibility
        is_first_player = self.current_player == 1
        is_second_player = self.current_player == 2

        # reset current player chip position array since we are going to rebuild it using the provided pos
        self.players[self.current_player - 1].pos = []

        # Get the current player's new pieces index
        for row_index, row in enumerate(pos):
            for col_index, col in enumerate(row):
                if (is_first_player and col == "W") or (is_second_player and col == "B"):
                    self.players[self.current_player -
                                 1].pos.append((row_index, col_index))
    ...
    def lose(self):
        ...
        """
        first_player_chips = self.players[0].pos
        second_player_chips = self.players[1].pos

        if self.current_player == 2:
            # The second player loses if any of the first player chips are in the black territory
            return any(item in self.black_territory for item in first_player_chips)

        # The first player loses if any of the second player chips are in the white terriroty
        return any(item in self.white_territory for item in second_player_chips)
